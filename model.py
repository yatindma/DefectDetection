...
class ResNet(nn.Module):
    def __init__(self, n_in_channels=3, n_out_classes=2):
        ...
        self.encoder_gate = nn.Sequential(
            nn.Conv2d(n_in_channels, out_channels=64, kernel_size=7, stride=2, padding=3, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1))

        self.resblock1 = ResBlock(in_ch=64, out_ch=64, stride=1)
        self.resblock2 = ResBlock(in_ch=64, out_ch=256, stride=2)
        self.resblock3 = ResBlock(in_ch=256, out_ch=512, stride=2)
        self.resblock4 = ResBlock(in_ch=512, out_ch=256, stride=2)
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.linear = nn.Linear(256, n_out_classes)

        # self.decoder = ResnetDecoder(in_ch=256, out_ch=n_out_classes)

    def forward(self, input_tensor):
        first_output = self.encoder_gate(input_tensor)
        ...
        mean = self.avg_pool(res_out4)
        flattened = torch.flatten(mean, 1)
        output_tensor = self.linear(flattened)
        sig_output_tensor = nn.functional.sigmoid(output_tensor)
        return sig_output_tensor
